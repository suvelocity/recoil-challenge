...
describe("first test", () => {
	...
	test("Can add background color functionality with normal state ", async () => {
		...
		const intialCanvasBackgoroundColor = await page.$eval(
			".canvas",
		...
		const afterClickCanvasBackgroundColor = await page.$eval(
			".canvas",
			e => getComputedStyle(e).backgroundColor
		);
		// compare the initial color to the color after the change
		expect(intialCanvasBackgoroundColor).not.toBe(
			afterClickCanvasBackgroundColor
		);
	});
